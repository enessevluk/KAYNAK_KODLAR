...
namespace Raven.MapGenerator
{
    ...
    internal sealed class RavenTerrainSafetyConfig
    {
        public bool FastCheck = true;
        public bool DeepCheck;
        public bool SafeAutoRepair = true;
        public int FastBudgetMs = 750;
        public int DeepBudgetMs = 2500;
    }

    internal sealed class RavenTerrainSafetyIssue
    {
    ...
    internal sealed class RavenTerrainSafetySummary
    {
        public bool Enabled;
        ...
        public bool HeightMapAvailable;
        public bool AlphaMapAvailable;
        public int SlopeObservations;
        public bool SafeAutoRepairEnabled;
        public int RepairsApplied;
        public int RepairCandidates;
        public string RepairPolicy = "verified_profiles_only";
        public List<RavenTerrainSafetyIssue> Issues = new List<RavenTerrainSafetyIssue>();
    }
    ...
    internal static class RavenWorldDataReporter
    {
        private static readonly string[] Categories = {
            ...
            "outpost", "bandit_camp", "stables", "harbor",
        ...
        private static object CachedTerrainHeightMap;
        private static MethodInfo CachedTerrainHeightMethod;
        private static bool TerrainHeightResolved;
        private static object CachedTerrainAlphaMap;
        private static MethodInfo CachedTerrainAlphaMethod;
        private static bool TerrainAlphaResolved;
        ...
        private static string ClassifyRootMonumentPrefab(string path)
        {
            ...
            if (PathEndsWithAny(n, "/monument/cave/cave_small_easy.prefab")) return "cave_small_easy";
            if (PathEndsWithAny(n, "/monument/cave/cave_medium_easy.prefab")) return "cave_medium_easy";
            if (PathEndsWithAny(n, "/monument/cave/cave_small_medium.prefab")) return "cave_small_medium";
            if (PathEndsWithAny(n, "/monument/cave/cave_medium_medium.prefab")) return "cave_medium_medium";
            if (PathEndsWithAny(n, "/monument/cave/cave_large_medium.prefab")) return "cave_large_medium";
            if (PathEndsWithAny(n, "/monument/cave/cave_small_hard.prefab")) return "cave_small_hard";
            if (PathEndsWithAny(n, "/monument/cave/cave_medium_hard.prefab")) return "cave_medium_hard";
            if (PathEndsWithAny(n, "/monument/cave/cave_large_hard.prefab")) return "cave_large_hard";
            if (PathEndsWithAny(n, "/monument/cave/cave_large_sewers_hard.prefab")) return "cave_large_sewers_hard";
        ...
        private static bool IsAuthoritativeRootMonumentCategory(string category)
        {
            switch (category)
            {
                ...
                case "sewer_branch": case "warehouse":
                ...
                case "underwater_labs":
                case "cave_small_easy": case "cave_medium_easy": case "cave_small_medium":
                case "cave_medium_medium": case "cave_large_medium": case "cave_small_hard":
                case "cave_medium_hard": case "cave_large_hard": case "cave_large_sewers_hard":
                case "water_wells": case "stone_quarry":
        ...
        private static string ClassifyPrefab(string path, string category)
        {
            ...
            if (ContainsAny(n, "/power substations/big/power_sub_big_1.prefab", "power_sub_big_1.prefab")) return "power_sub_big_1";
            if (ContainsAny(n, "/power substations/big/power_sub_big_2.prefab", "power_sub_big_2.prefab")) return "power_sub_big_2";
            if (ContainsAny(n, "/power substations/small/power_sub_small_1.prefab", "power_sub_small_1.prefab")) return "power_sub_small_1";
            if (ContainsAny(n, "/power substations/small/power_sub_small_2.prefab", "power_sub_small_2.prefab")) return "power_sub_small_2";
            ...
            // RustEdit adı `swamp_c` olsa da bu prefab bataklık değil, ayrı bir
            // küçük monument olan Abandoned Cabins'tir. Geniş swamp ailesinden
            // önce ayır; aksi halde yanlış bataklık ikonu basılır.
            if (ContainsAny(n, "/monument/swamp/swamp_c.prefab", "/monument/swamp/swamp_c", "swamp_c.prefab")) return "abandoned_cabins";
            if (ContainsAny(n, "/monument/swamp/swamp_a.prefab", "/monument/swamp/swamp_b.prefab", "/unique_environment/jungle/ue_jungle_swamp_")) return "swamps";
        ...
        private static string DisplayName(string category)
        {
            switch (category)
            {
                ...
                case "cave_small_easy": return "Cave Small Easy";
                case "cave_medium_easy": return "Cave Medium Easy";
                case "cave_small_medium": return "Cave Small Medium";
                case "cave_medium_medium": return "Cave Medium Medium";
                case "cave_large_medium": return "Cave Large Medium";
                case "cave_small_hard": return "Cave Small Hard";
                case "cave_medium_hard": return "Cave Medium Hard";
                case "cave_large_hard": return "Cave Large Hard";
                case "cave_large_sewers_hard": return "Cave Large Sewers Hard";
                ...
                case "power_sub_big_1": return "Power Substation Big 1";
                case "power_sub_big_2": return "Power Substation Big 2";
                case "power_sub_small_1": return "Power Substation Small 1";
                case "power_sub_small_2": return "Power Substation Small 2";
        ...
        private static bool ReadJsonBoolean(string json, string name, bool fallback)
        {
            if (String.IsNullOrEmpty(json) || String.IsNullOrEmpty(name)) return fallback;
            int key = json.IndexOf("\"" + name + "\"", StringComparison.OrdinalIgnoreCase);
            if (key < 0) return fallback;
            int colon = json.IndexOf(':', key + name.Length + 2);
            if (colon < 0) return fallback;
            int start = colon + 1;
            while (start < json.Length && Char.IsWhiteSpace(json[start])) start++;
            if (start + 4 <= json.Length && String.Compare(json, start, "true", 0, 4, true, CultureInfo.InvariantCulture) == 0) return true;
            if (start + 5 <= json.Length && String.Compare(json, start, "false", 0, 5, true, CultureInfo.InvariantCulture) == 0) return false;
            return fallback;
        }

        private static int ReadJsonInteger(string json, string name, int fallback, int min, int max)
        {
            if (String.IsNullOrEmpty(json) || String.IsNullOrEmpty(name)) return fallback;
            int key = json.IndexOf("\"" + name + "\"", StringComparison.OrdinalIgnoreCase);
            if (key < 0) return fallback;
            ...
            int parsed;
            if (end == start || !Int32.TryParse(json.Substring(start, end - start), NumberStyles.Integer, CultureInfo.InvariantCulture, out parsed)) return fallback;
            return Math.Max(min, Math.Min(max, parsed));
        }

        private static RavenTerrainSafetyConfig ReadTerrainSafetyConfig(List<string> diagnostics)
        {
            RavenTerrainSafetyConfig result = new RavenTerrainSafetyConfig();
            try
            {
                string path = Path.GetFullPath(Path.Combine("HarmonyConfig", "RavenGuaranteedPlacement.json"));
                if (!File.Exists(path))
                {
                    diagnostics.Add("Terrain safety config missing; fast defaults used.");
                    return result;
                }
                string json = File.ReadAllText(path);
                result.FastCheck = ReadJsonBoolean(json, "fastCheck", true);
                result.DeepCheck = ReadJsonBoolean(json, "deepCheck", false);
                result.SafeAutoRepair = ReadJsonBoolean(json, "safeAutoRepair", true);
                result.FastBudgetMs = ReadJsonInteger(json, "fastBudgetMs", 750, 100, 1500);
                result.DeepBudgetMs = ReadJsonInteger(json, "deepBudgetMs", 2500, 500, 4000);
            }
            catch (Exception ex)
            {
                diagnostics.Add("Terrain safety config read failed; fast defaults used: " + ex.GetType().Name);
            }
            return result;
        }

        private static bool IsAuditableRockRoot(string path)
        {
            string normalized = Normalize(path).Trim();
            int slash = normalized.LastIndexOf('/');
            string file = slash >= 0 ? normalized.Substring(slash + 1) : normalized;
            const string suffix = ".prefab";
            if (!file.EndsWith(suffix, StringComparison.Ordinal)) return false;

            // Rust currently serializes ordinary terrain rocks through two root
            // families. Keep this as an explicit allow-list: cave, God Rock,
            // monument and reusable huge child prefabs must never enter the
            // automatic terrain audit merely because their name contains "rock".
            bool v3Root = normalized.IndexOf("/decor/v3_rock_formations_", StringComparison.Ordinal) >= 0
                && HasSingleLetterRockVariant(file, "rock_formation_", suffix);
            bool hillCliffRoot = normalized.IndexOf("/decor/hill_cliffs_small/", StringComparison.Ordinal) >= 0
                && (HasSingleLetterRockVariant(file, "rock_formation_small_", suffix)
                    || HasSingleLetterRockVariant(file, "rock_formation_medium_", suffix)
                    || HasSingleLetterRockVariant(file, "rock_formation_large_", suffix));
            return v3Root || hillCliffRoot;
        }

        private static bool HasSingleLetterRockVariant(string file, string prefix, string suffix)
        {
            if (!file.StartsWith(prefix, StringComparison.Ordinal) || !file.EndsWith(suffix, StringComparison.Ordinal)) return false;
            string variant = file.Substring(prefix.Length, file.Length - prefix.Length - suffix.Length);
            return variant.Length == 1 && Char.IsLetter(variant[0]);
        }

        private static bool IsProtectedRockIdentity(RavenGodRockCandidate candidate)
        {
            string value = Normalize((candidate.Path ?? String.Empty) + " | " + (candidate.Category ?? String.Empty));
            return candidate.StrongIdentity || candidate.HugeIdentity || ContainsAny(value,
                "godrock", "god_rock", "/cave", "cave_", "/arch", "tunnel", "underground", "entrance", "monument");
        }

        private static float RockAuditRadius(string path)
        {
            string normalized = Normalize(path);
            if (normalized.IndexOf("rock_formation_small_", StringComparison.Ordinal) >= 0) return 7f;
            if (normalized.IndexOf("rock_formation_medium_", StringComparison.Ordinal) >= 0) return 13f;
            if (normalized.IndexOf("rock_formation_large_", StringComparison.Ordinal) >= 0) return 24f;
            if (normalized.IndexOf("_small/", StringComparison.Ordinal) >= 0) return 7f;
            if (normalized.IndexOf("_medium/", StringComparison.Ordinal) >= 0) return 13f;
            if (normalized.IndexOf("_large/", StringComparison.Ordinal) >= 0) return 24f;
            return 14f;
        }

        private static float RockAuditRangeLimit(string path)
        {
            string normalized = Normalize(path);
            if (normalized.IndexOf("rock_formation_small_", StringComparison.Ordinal) >= 0) return 7f;
            if (normalized.IndexOf("rock_formation_medium_", StringComparison.Ordinal) >= 0) return 12f;
            if (normalized.IndexOf("rock_formation_large_", StringComparison.Ordinal) >= 0) return 20f;
            if (normalized.IndexOf("_small/", StringComparison.Ordinal) >= 0) return 7f;
            if (normalized.IndexOf("_medium/", StringComparison.Ordinal) >= 0) return 12f;
            if (normalized.IndexOf("_large/", StringComparison.Ordinal) >= 0) return 20f;
            return 13f;
        }

        private static bool TryGetTerrainHeight(float x, float z, uint worldSize, out float height)
        {
            height = 0f;
            try
            {
                ...
                {
                    TerrainHeightResolved = true;
                    Type terrainMeta = AccessTools.TypeByName("TerrainMeta");
                    CachedTerrainHeightMap = terrainMeta == null ? null : GetMember(terrainMeta, "HeightMap", "heightMap");
                    if (CachedTerrainHeightMap != null)
                    {
                        CachedTerrainHeightMethod = CachedTerrainHeightMap.GetType().GetMethod("GetHeight",
                            BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, null,
                            new Type[] { typeof(float), typeof(float) }, null);
                    }
                }
                if (CachedTerrainHeightMap == null || CachedTerrainHeightMethod == null || worldSize == 0U) return false;
                float u = x / worldSize + 0.5f;
                float v = z / worldSize + 0.5f;
                object value = CachedTerrainHeightMethod.Invoke(CachedTerrainHeightMap, new object[] { u, v });
                if (value == null) return false;
                height = Convert.ToSingle(value, CultureInfo.InvariantCulture);
                return !Single.IsNaN(height) && !Single.IsInfinity(height);
            }
            catch { return false; }
        }

        private static bool TryGetTerrainAlpha(float x, float z, uint worldSize, out float alpha)
        {
            alpha = 1f;
            try
            {
                if (!TerrainAlphaResolved)
                {
                    TerrainAlphaResolved = true;
                    Type terrainMeta = AccessTools.TypeByName("TerrainMeta");
                    CachedTerrainAlphaMap = terrainMeta == null ? null : GetMember(terrainMeta, "AlphaMap", "alphaMap");
                    if (CachedTerrainAlphaMap != null)
                    {
                        CachedTerrainAlphaMethod = CachedTerrainAlphaMap.GetType().GetMethod("GetAlpha",
                            BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, null,
                            new Type[] { typeof(float), typeof(float) }, null);
                    }
                }
                if (CachedTerrainAlphaMap == null || CachedTerrainAlphaMethod == null || worldSize == 0U) return false;
                float u = x / worldSize + 0.5f;
                float v = z / worldSize + 0.5f;
                object value = CachedTerrainAlphaMethod.Invoke(CachedTerrainAlphaMap, new object[] { u, v });
                if (value == null) return false;
                alpha = Convert.ToSingle(value, CultureInfo.InvariantCulture);
                return !Single.IsNaN(alpha) && !Single.IsInfinity(alpha);
            }
            catch { return false; }
        }

        private static RavenTerrainSafetySummary AuditRockTerrain(List<RavenGodRockCandidate> candidates, uint worldSize, List<string> diagnostics)
        {
            RavenTerrainSafetyConfig config = ReadTerrainSafetyConfig(diagnostics);
            RavenTerrainSafetySummary summary = new RavenTerrainSafetySummary();
            summary.Enabled = config.FastCheck || config.DeepCheck;
            summary.Mode = config.DeepCheck ? "deep" : config.FastCheck ? "fast" : "disabled";
            summary.BudgetMs = config.DeepCheck ? config.DeepBudgetMs : config.FastBudgetMs;
            summary.SafeAutoRepairEnabled = config.SafeAutoRepair;
            if (!summary.Enabled)
            {
                diagnostics.Add("Terrain safety audit disabled by project settings.");
                return summary;
            }

            List<RavenGodRockCandidate> roots = new List<RavenGodRockCandidate>();
            for (int i = 0; i < candidates.Count; i++)
                if (IsAuditableRockRoot(candidates[i].Path)) roots.Add(candidates[i]);
            summary.Candidates = roots.Count;

            // Deep mode is expected to be exhaustive. The elapsed-time budget is
            // still the hard safety valve, so a future Rust map with an unusually
            // large prefab count cannot stall generation. Fast mode intentionally
            // samples at most 700 roots.
            int limit = config.DeepCheck ? Int32.MaxValue : 700;
            int stride = roots.Count > limit ? (int)Math.Ceiling(roots.Count / (double)limit) : 1;
            int ringSamples = config.DeepCheck ? 12 : 4;
            Stopwatch timer = Stopwatch.StartNew();
            for (int index = 0; index < roots.Count; index += stride)
            {
                if (timer.ElapsedMilliseconds >= summary.BudgetMs)
                {
                    summary.BudgetExceeded = true;
                    break;
                }

                RavenGodRockCandidate candidate = roots[index];
                if (IsProtectedRockIdentity(candidate)) summary.Protected++;
                float center;
                if (!TryGetTerrainHeight(candidate.Position.x, candidate.Position.z, worldSize, out center)) continue;
                summary.HeightMapAvailable = true;
                float min = center, max = center;
                float radius = RockAuditRadius(candidate.Path);
                int alphaHoleSamples = 0;
                float centerAlpha;
                if (TryGetTerrainAlpha(candidate.Position.x, candidate.Position.z, worldSize, out centerAlpha))
                {
                    summary.AlphaMapAvailable = true;
                    if (centerAlpha < 0.1f) alphaHoleSamples++;
                }
                for (int sample = 0; sample < ringSamples; sample++)
                {
                    double angle = sample * Math.PI * 2.0 / ringSamples;
                    float sampleX = candidate.Position.x + (float)Math.Cos(angle) * radius;
                    float sampleZ = candidate.Position.z + (float)Math.Sin(angle) * radius;
                    float value;
                    if (TryGetTerrainHeight(sampleX, sampleZ, worldSize, out value))
                    {
                        if (value < min) min = value;
                        if (value > max) max = value;
                    }
                    float sampleAlpha;
                    if (TryGetTerrainAlpha(sampleX, sampleZ, worldSize, out sampleAlpha))
                    {
                        summary.AlphaMapAvailable = true;
                        if (sampleAlpha < 0.1f) alphaHoleSamples++;
                    }
                }
                summary.Checked++;
                float range = max - min;
                float allowed = RockAuditRangeLimit(candidate.Path);
                if (range > allowed) summary.SlopeObservations++;

                // A steep surface is normal for cliff formations and is not an
                // actionable defect by itself. A terrain alpha hole underneath an
                // ordinary serialized rock is the high-confidence signal: Rust's
                // own nav-mesh code treats alpha < 0.1 as absent terrain.
                if (alphaHoleSamples == 0) continue;

                summary.RepairCandidates++;
                if (summary.Issues.Count >= 40) continue;
                int slash = candidate.Path == null ? -1 : candidate.Path.LastIndexOf('/');
                string name = slash >= 0 ? candidate.Path.Substring(slash + 1).Replace(".prefab", String.Empty) : candidate.Path;
                summary.Issues.Add(new RavenTerrainSafetyIssue {
                    Severity = alphaHoleSamples >= 2 ? "high" : "warning",
                    Name = name ?? "rock_formation",
                    Path = candidate.Path ?? String.Empty,
                    X = candidate.Position.x, Y = candidate.Position.y, Z = candidate.Position.z,
                    TerrainRange = range,
                    Message = IsProtectedRockIdentity(candidate)
                        ? "Korunan taş kimliği Alpha Map deliğiyle çakışıyor; otomatik değişiklik yapılmadı."
                        : "Taş tabanı Alpha Map deliğiyle çakışıyor; oyun içinde girilebilir boşluk riski var."
                });
            }
            summary.ElapsedMs = timer.ElapsedMilliseconds;
            // No unknown prefab is moved, hidden or deleted. The repair switch is
            // intentionally profile-gated; profiles can be added after a specific
            // Rust prefab has been collision-tested.
            summary.RepairsApplied = 0;
            diagnostics.Add("Terrain safety: mode=" + summary.Mode
                + ", candidates=" + summary.Candidates.ToString(CultureInfo.InvariantCulture)
                + ", checked=" + summary.Checked.ToString(CultureInfo.InvariantCulture)
                + ", slopes=" + summary.SlopeObservations.ToString(CultureInfo.InvariantCulture)
                + ", alphaAvailable=" + summary.AlphaMapAvailable.ToString()
                + ", review=" + summary.RepairCandidates.ToString(CultureInfo.InvariantCulture)
                + ", repairs=" + summary.RepairsApplied.ToString(CultureInfo.InvariantCulture)
                + ", budgetExceeded=" + summary.BudgetExceeded.ToString()
                + ", ms=" + summary.ElapsedMs.ToString(CultureInfo.InvariantCulture));
            return summary;
        }
        ...
        private static bool CollectWorldPrefabs(object worldData, List<RavenWorldEntry> entries, HashSet<string> seen, List<string> diagnostics,
            bool collectGodRockFallback, uint worldSize, out RavenTerrainSafetySummary terrainSafety)
        {
            terrainSafety = new RavenTerrainSafetySummary();
            ...
            List<RavenGodRockCandidate> terrainAuditCandidates = new List<RavenGodRockCandidate>();
            Stopwatch timer = Stopwatch.StartNew();
            foreach (object prefab in prefabs)
            {
                ...
                if (genericRockCandidate && hasPosition && IsAuditableRockRoot(path) && terrainAuditCandidates.Count < 50000)
                {
                    terrainAuditCandidates.Add(new RavenGodRockCandidate {
            ...
            if (collectGodRockFallback)
            {
                ...
                int sampleCount = Math.Min(godRockCandidates.Count, 40);
            ...
            terrainSafety = AuditRockTerrain(terrainAuditCandidates, worldSize, diagnostics);
        ...
        private static void WriteReportFile(string phase, bool complete, string stage, uint worldSize, uint seed, string identity,
            ...
            bool runtimeMonumentsAvailable, bool powerlinesAvailable, bool topologyAvailable, int powerlineCount,
            Dictionary<string, int> topologyCounts, RavenTerrainSafetySummary terrainSafety)
        {
            ...
            RavenTerrainSafetySummary safety = terrainSafety ?? new RavenTerrainSafetySummary();
            json.Append("  ],\n  \"terrainSafety\": {");
            json.Append("\"enabled\": ").Append(safety.Enabled ? "true" : "false");
            json.Append(", \"mode\": \"").Append(Escape(safety.Mode)).Append("\"");
            json.Append(", \"budgetMs\": ").Append(safety.BudgetMs.ToString(CultureInfo.InvariantCulture));
            json.Append(", \"elapsedMs\": ").Append(safety.ElapsedMs.ToString(CultureInfo.InvariantCulture));
            json.Append(", \"candidates\": ").Append(safety.Candidates.ToString(CultureInfo.InvariantCulture));
            json.Append(", \"checked\": ").Append(safety.Checked.ToString(CultureInfo.InvariantCulture));
            json.Append(", \"protected\": ").Append(safety.Protected.ToString(CultureInfo.InvariantCulture));
            json.Append(", \"budgetExceeded\": ").Append(safety.BudgetExceeded ? "true" : "false");
            json.Append(", \"heightMapAvailable\": ").Append(safety.HeightMapAvailable ? "true" : "false");
            json.Append(", \"alphaMapAvailable\": ").Append(safety.AlphaMapAvailable ? "true" : "false");
            json.Append(", \"slopeObservations\": ").Append(safety.SlopeObservations.ToString(CultureInfo.InvariantCulture));
            json.Append(", \"safeAutoRepairEnabled\": ").Append(safety.SafeAutoRepairEnabled ? "true" : "false");
            json.Append(", \"repairsApplied\": ").Append(safety.RepairsApplied.ToString(CultureInfo.InvariantCulture));
            json.Append(", \"repairCandidates\": ").Append(safety.RepairCandidates.ToString(CultureInfo.InvariantCulture));
            json.Append(", \"repairPolicy\": \"").Append(Escape(safety.RepairPolicy)).Append("\"");
            json.Append(", \"issues\": [");
            for (int i = 0; i < safety.Issues.Count; i++)
            {
                if (i > 0) json.Append(',');
                RavenTerrainSafetyIssue issue = safety.Issues[i];
                json.Append("{\"severity\":\"").Append(Escape(issue.Severity)).Append("\",\"name\":\"").Append(Escape(issue.Name));
                json.Append("\",\"path\":\"").Append(Escape(issue.Path)).Append("\",\"x\":").Append(issue.X.ToString("R", CultureInfo.InvariantCulture));
                json.Append(",\"y\":").Append(issue.Y.ToString("R", CultureInfo.InvariantCulture)).Append(",\"z\":").Append(issue.Z.ToString("R", CultureInfo.InvariantCulture));
                json.Append(",\"terrainRange\":").Append(issue.TerrainRange.ToString("R", CultureInfo.InvariantCulture));
        ...
        public static void WriteReport(string phase, bool force)
        {
            lock (Sync)
            {
                ...
                WriteReportFile(phase, false, "started", worldSize, seed, identity, mapPath, entries, diagnostics,
                    false, false, false, false, 0, topologyCounts, null);
                ...
                // The serialized map root scan below is authoritative. The former
                // transform-wide named-root fallback inspected hundreds of thousands
                // of objects, produced no accepted roots and cost up to eight seconds.
                diagnostics.Add("Runtime explicit God Rock scene scan skipped: saved-map root identities are authoritative.");
                ...
                stage.Restart();
                RavenTerrainSafetySummary terrainSafety;
                bool prefabsAvailable = CollectWorldPrefabs(savedWorld, entries, seen, diagnostics, true, worldSize, out terrainSafety);
                ...
                WriteReportFile(phase, false, "prefabs_powerlines_complete", worldSize, seed, identity, mapPath, entries, diagnostics,
                    prefabsAvailable, runtimeMonumentsAvailable, powerlinesAvailable, false, powerlineCount, topologyCounts, terrainSafety);
                ...
                WriteReportFile(phase, finalComplete, finalStage, worldSize, seed, identity, mapPath, entries, diagnostics,
                    prefabsAvailable, runtimeMonumentsAvailable, powerlinesAvailable, topologyAvailable, powerlineCount, topologyCounts, terrainSafety);

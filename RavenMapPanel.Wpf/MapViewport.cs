...
public sealed class MapViewport : FrameworkElement
{
    ...
    private Color oceanColor = Color.FromRgb(0x0A, 0x3B, 0x49);
    ...
    public MapDocument? Document
    {
        ...
        {
            document = value;
            oceanColor = ResolveOceanColor(value?.Image);
            selectedMarker = null;
            Fit();
    ...
    {
        var hasMap = document?.Image is not null;
        var background = hasMap
            ? oceanColor
            : Color.FromRgb(6, 17, 23);
        dc.DrawRectangle(new SolidColorBrush(background), null, new Rect(RenderSize));
        if (!hasMap)
        {
            DrawGrid(dc);
            DrawEmpty(dc);
        ...
        var imageRect = new Rect(origin.X, origin.Y, current.ImageWidth * scale, current.ImageHeight * scale);
        dc.PushClip(new RectangleGeometry(new Rect(RenderSize))); dc.DrawImage(current.Image, imageRect);
        if (ShowIcons)
        {
            foreach (var marker in current.Markers)
            {
                if (!ShowEnvironment && IsEnvironment(marker.Category)) continue;
                var p = current.Project(marker.X, marker.Z); var screen = new Point(origin.X + p.X * scale, origin.Y + p.Y * scale);
    ...
    private (double Scale, Point Origin) Transform()
    {
        ...
        var fit = Math.Min(ActualWidth / document.ImageWidth, ActualHeight / document.ImageHeight) * .995;
        var scale = fit * zoom;
    ...
    private void SetZoom(double next, Point pivot)
    {
        ...
        zoom = next; var fit = Math.Min(ActualWidth / document.ImageWidth, ActualHeight / document.ImageHeight) * .995; var newScale = fit * zoom;
    ...
    private static Color ResolveOceanColor(ImageSource? image)
    {
        var fallback = Color.FromRgb(0x0A, 0x3B, 0x49);
        if (image is not BitmapSource source || source.PixelWidth < 1 || source.PixelHeight < 1)
            return fallback;
        ...
        try
        {
            ...
            var points = new[]
            {
                ...
                new Int32Rect(bitmap.PixelWidth - 1, 0, 1, 1),
                new Int32Rect(0, bitmap.PixelHeight - 1, 1, 1),
                new Int32Rect(bitmap.PixelWidth - 1, bitmap.PixelHeight - 1, 1, 1)
            };
            ...
            return Color.FromRgb((byte)(red / points.Length), (byte)(green / points.Length), (byte)(blue / points.Length));
        }
        catch
        {
            return fallback;
    ...
    private void DrawHud(DrawingContext dc, double scale, Point origin)
    {
        var label = $"{ZoomPercent:0}%";
        if (document is not null && scale > 0)
        {
            var sourceX = (mousePoint.X - origin.X) / scale;
            var sourceY = (mousePoint.Y - origin.Y) / scale;
            if (sourceX >= 0 && sourceY >= 0 && sourceX <= document.ImageWidth && sourceY <= document.ImageHeight)
            {
                var worldX = ((sourceX - document.RenderOffset) / document.MapResolution) * document.WorldSize - document.WorldSize / 2d;
                var worldZ = document.WorldSize / 2d - ((sourceY - document.RenderOffset) / document.MapResolution) * document.WorldSize;
                label = $"X {worldX:0}  Z {worldZ:0}   {ZoomPercent:0}%";
            }
            else
            {
                label = $"Deniz görünümü   {ZoomPercent:0}%";
            }
        }
        var text = new FormattedText(label, System.Globalization.CultureInfo.CurrentCulture, FlowDirection.LeftToRight, new Typeface("Cascadia Mono"), 11, new SolidColorBrush(Color.FromRgb(190, 205, 216)), VisualTreeHelper.GetDpi(this).PixelsPerDip);
        var rect = new Rect(ActualWidth - text.Width - 26, ActualHeight - 35, text.Width + 16, 25);
        dc.DrawRoundedRectangle(new SolidColorBrush(Color.FromArgb(220, 8, 13, 18)), null, rect, 5, 5);
        dc.DrawText(text, new Point(rect.X + 8, rect.Y + 5));
    }
    ...
    private double DisplayIconSide(string category, double scale)
    {
        ...
        return Math.Clamp(sourcePixels * scale, compact ? 4 : 6, compact ? 24 : 44);
    }
    ...
    private static bool IsEnvironment(string id) => id == "caves" || id.StartsWith("cave_",StringComparison.OrdinalIgnoreCase) || id.StartsWith("power_sub_",StringComparison.OrdinalIgnoreCase) || id is "canyons" or "lakes" or "oases" or "icebergs" or "powerlines" or "power_substations" or "metro_entrances" or "god_rocks";
}

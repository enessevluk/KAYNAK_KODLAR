...
namespace CustomGenerator.Generators
{
    ...
    class PlaceMonuments_Process {
        ...
        private static bool Prefix(PlaceMonuments __instance) {
            ...
            var matchMonuments = Config.Monuments.monuments.Where(x =>
                x != null && string.Equals(x.Folder, __instance.ResourceFolder, StringComparison.OrdinalIgnoreCase));
    ...
    class WorldSetup_InitCoroutine
    {
        ...
        private static bool Prefix(WorldSetup __instance) {
            ...
            PlaceMonuments[] placeMonuments = __instance.GetComponentsInChildren<ProceduralComponent>(true).OfType<PlaceMonuments>().ToArray();
            Logging.Info($"Found {placeMonuments.Length} PlaceMonuments.");

...
namespace CustomGenerator.Utility {
    ...
    public static class MapImageRender {
        ...
        private static void LoadIcons(ref Array2D<Color> output, int imageWidth, int imageHeight, int mapResolution, int oceanMargin) {
            ...
            // Raven V3 renders normalized monument icons as a separate vector layer.
            // Keeping the legacy black markers here creates duplicate and apparently offset icons.
            // We still collect mapMonuments above so reporter Harmony patches receive identical data.
            Logging.Info("Raven V3 | Base map rendered without legacy monument glyphs.");
        }
